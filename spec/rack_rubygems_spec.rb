...
describe "The Rack Rubygems Server" do
  ...
  describe "serves some static files" do
    ...
    it 'including rdocs' do
      get "/doc_root/rubygems-#{Gem::VERSION}/rdoc/index.html" #rubygems-1.3.1
  ...
  describe "provides access to individual gemspecs" do
    let(:gem_name) { 'sinatra-0.9.0.4' }
    
    it "via name and version" do
      should_match_webrick_behavior "/quick/#{gem_name}.gemspec.rz", :quick
    end

    it "via name, version, and platform" do
      should_match_webrick_behavior "/quick/#{gem_name}-ruby.gemspec.rz", :quick
    end
    ...
    it "marshalled via name and version" do
      should_match_webrick_behavior "/quick/Marshal.#{Gem.marshal_version}/#{gem_name}.gemspec.rz", :quick
    end

    it "marshalled via name, version, and platform" do
      should_match_webrick_behavior "/quick/Marshal.#{Gem.marshal_version}/#{gem_name}-ruby.gemspec.rz", :quick
    end
  end
